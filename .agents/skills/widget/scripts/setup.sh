...
TARGET_SKILL_DIR="$CODEX_HOME/skills/widget"
INSTALLED_SKILL_FILE="$TARGET_SKILL_DIR/SKILL.md"

echo ""
echo "WidgetDesk Skill Setup (Codex)"
echo "=============================="
echo ""
...
if [ "$CHECK_ONLY" -eq 1 ]; then
  ...
  if [ -f "$INSTALLED_SKILL_FILE" ]; then
    echo -e "${GREEN}✓${NC} 已检测到已安装的 Codex skill"
  else
    echo -e "${YELLOW}→${NC} 尚未安装 Codex skill 到 $TARGET_SKILL_DIR"
  fi
  echo ""
  echo "检查完成。"
  if [ ! -f "$INSTALLED_SKILL_FILE" ]; then
    ...
    echo "下一步：让 agent 使用 widget skill 创建一个组件"
  fi
  exit 0
fi
...
if [ "$SOURCE_SKILL_DIR" != "$TARGET_SKILL_DIR" ]; then
  ...
  echo -e "${GREEN}✓${NC} Codex skill 已安装 → $TARGET_SKILL_DIR"
else
  chmod +x "$TARGET_SKILL_DIR/scripts/"*.sh
  echo -e "${GREEN}✓${NC} 正在使用已安装的 Codex skill：$TARGET_SKILL_DIR"
fi
...
echo ""
echo "诊断命令："
echo "  bash \"${CODEX_HOME:-$HOME/.codex}/skills/widget/scripts/doctor.sh\""
echo ""
echo "下一步：让 agent 使用 widget skill 创建或修改桌面组件"
echo ""
